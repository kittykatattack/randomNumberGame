...
Elm.Game.make = function (_elm) {
   ...
   var inputField = function (address) {
      ...
      _U.list([$Html$Attributes.placeholder("Enter a number...")
              ...
              ,A3($Html$Events.on,"change",$Html$Events.targetValue,function (_p0) {    return A2($Signal.message,address,EnterText(_p0));})]),
      _U.list([]));
   };
   ...
   var init = {ctor: "_Tuple2",_0: {mysteryNumber: 50,maxGuesses: 10,guessesMade: 0,gameState: Started,inputValue: 0},_1: $Effects.none};
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "Guess": var checkGameState = function (model) {
              return _U.cmp(model.guessesMade,model.maxGuesses) > -1 && !_U.eq(model.inputValue,model.mysteryNumber) ? Lost : _U.cmp(model.guessesMade,
              model.maxGuesses) < 1 && _U.eq(model.inputValue,model.mysteryNumber) ? Won : InProgress;
           };
           return {ctor: "_Tuple2",_0: _U.update(model,{guessesMade: model.guessesMade + 1,gameState: checkGameState(model)}),_1: $Effects.none};
         case "EnterText": return {ctor: "_Tuple2"
                                  ,_0: _U.update(model,{inputValue: A2($Maybe.withDefault,0,$Result.toMaybe($String.toInt(_p1._0)))})
                                  ,_1: $Effects.none};
         default: return init;}
   });
   ...
   var gameMessage = function (model) {
      var stateMessage = function (model) {
         ...
         " Your guess: ",
         A2($Basics._op["++"],
         $Basics.toString(model.inputValue),
         A2($Basics._op["++"],
         ", State: ",
         A2($Basics._op["++"],
         $Basics.toString(model.gameState),
         A2($Basics._op["++"],
         ", Mystery Number: ",
         A2($Basics._op["++"],
         $Basics.toString(model.mysteryNumber),
         _U.eq(model.gameState,Started) || _U.eq(model.gameState,InProgress) ? A2($Basics._op["++"],
         ", Guess Number: ",
         A2($Basics._op["++"],$Basics.toString(model.guessesMade),A2($Basics._op["++"],", Max Guesses:  ",$Basics.toString(model.maxGuesses)))) : ""))))));
      };
