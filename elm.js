...
Elm.Game.make = function (_elm) {
   ...
   var intOrZero = function (i) {    return A2($Maybe.withDefault,0,$Result.toMaybe($String.toInt(i)));};
   var newRandIntMB = $Signal.mailbox({ctor: "_Tuple0"});
   var NewMystery = function (a) {    return {ctor: "NewMystery",_0: a};};
   var toAction = function (_p0) {
      var _p1 = _p0;
      var _p2 = A2($Random.generate,A2($Random.$int,1,100),$Random.initialSeed($Basics.truncate(_p1._0)));
      var newInt = _p2._0;
      return NewMystery(newInt);
   };
   var newRandIntSignal = A2($Signal.map,toAction,$Time.timestamp(newRandIntMB.signal));
   var EnterText = function (a) {    return {ctor: "EnterText",_0: a};};
   var newIntEffect = $Effects.task(A2($Task.map,$Basics.always(EnterText("")),A2($Signal.send,newRandIntMB.address,{ctor: "_Tuple0"})));
   var inputField = function (address) {
      return A2($Html.input,
      _U.list([$Html$Attributes.placeholder("Enter a number...")
              ,$Html$Attributes.type$("number")
              ,A3($Html$Events.on,"change",$Html$Events.targetValue,function (_p3) {    return A2($Signal.message,address,EnterText(_p3));})]),
      _U.list([]));
   };
   var Restart = {ctor: "Restart"};
   var Guess = {ctor: "Guess"};
   var Model = F5(function (a,b,c,d,e) {    return {mysteryNumber: a,maxGuesses: b,guessesMade: c,gameState: d,inputValue: e};});
   var InProgress = {ctor: "InProgress"};
   var Won = {ctor: "Won"};
   var Lost = {ctor: "Lost"};
   var Started = {ctor: "Started"};
   var init = {ctor: "_Tuple2",_0: {mysteryNumber: 50,maxGuesses: 10,guessesMade: 0,gameState: Started,inputValue: 0},_1: newIntEffect};
   var update = F2(function (action,model) {
      var _p4 = action;
      switch (_p4.ctor)
      {case "Guess": var checkGameState = function (model) {
              var _p5 = {ctor: "_Tuple2",_0: _U.cmp(model.guessesMade,model.maxGuesses) > -1,_1: _U.eq(model.inputValue,model.mysteryNumber)};
              _v2_2: do {
                 if (_p5.ctor === "_Tuple2") {
                       if (_p5._0 === true) {
                             return Lost;
           ...
           return noFx(_U.update(model,{guessesMade: model.guessesMade + 1,gameState: checkGameState(model)}));
         case "EnterText": return noFx(_U.update(model,{inputValue: intOrZero(_p4._0)}));
         case "NewMystery": return noFx(_U.update(model,{mysteryNumber: _p4._0}));
         default: return init;}
   });
   var guessButton = F2(function (address,model) {
      return _U.eq(model.gameState,Started) || _U.eq(model.gameState,InProgress) ? A2($Html.button,
      _U.list([A2($Html$Events.onClick,address,Guess)]),
      _U.list([$Html.text("guess")])) : A2($Html.button,_U.list([A2($Html$Events.onClick,address,Restart)]),_U.list([$Html.text("restart")]));
   });
   var gameMessage = function (model) {
      var stateMessage = function (model) {
         var stateToString = _U.eq(model.gameState,Started) || _U.eq(model.gameState,InProgress) ? A2($String$Interpolate.interpolate,
         ", Guess Number: {0} , Max Guesses: {1}",
         _U.list([$Basics.toString(model.guessesMade),$Basics.toString(model.maxGuesses)])) : "";
         return A2($String$Interpolate.interpolate,
         " Your guess: {0}, State: {1}, Mystery Number: {2} {3}",
         _U.list([$Basics.toString(model.inputValue),$Basics.toString(model.gameState),$Basics.toString(model.mysteryNumber),stateToString]));
      };
      var _p6 = model.gameState;
      switch (_p6.ctor)
      {case "Started": return A2($Basics._op["++"],"I am thinking of a number between 1 and 100.",stateMessage(model));
         case "InProgress": return _U.cmp(model.inputValue,model.mysteryNumber) < 0 ? A2($Basics._op["++"],
           "That\'s too low.",
           stateMessage(model)) : A2($Basics._op["++"],"That\'s too high.",stateMessage(model));
         case "Lost": return A2($Basics._op["++"],
           "You\'ve run out of guesses! The mystery number was: ",
           A2($Basics._op["++"],$Basics.toString(model.mysteryNumber),A2($Basics._op["++"],". ",A2($Basics._op["++"],"Game Over!",stateMessage(model)))));
         default: return A2($Basics._op["++"],
           "That\'s correct! It took you: ",
           A2($Basics._op["++"],
           $Basics.toString(model.guessesMade),
           A2($Basics._op["++"]," guesses. ",A2($Basics._op["++"],"Game Over!",stateMessage(model)))));}
   };
   var messageDisplay = function (model) {    return A2($Html.p,_U.list([]),_U.list([$Html.text(gameMessage(model))]));};
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([]),_U.list([messageDisplay(model),inputField(address),A2(guessButton,address,model)]));
   });
   return _elm.Game.values = {_op: _op
                             ...
                             ,newRandIntMB: newRandIntMB
                             ,toAction: toAction
                             ,newRandIntSignal: newRandIntSignal
                             ,newIntEffect: newIntEffect
                             ,intOrZero: intOrZero
                             ,noFx: noFx
                             ,update: update
                             ,view: view
...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $Game.init,update: $Game.update,view: $Game.view,inputs: _U.list([$Game.newRandIntSignal])});
   var main = app.html;
